...
- `lib/deployTools/mkCompactClosure.nix` reads that references file, filters it, and returns a `symlinkJoin`. Extend the default filters here, not in `mkBundle`.

## mkBundle Contract
- `mkBundle` now depends on `deployTools.mkCompactClosure`; keep `referenceExcludes` logic in sync with that helper. The attr set is `{ useDefaults ? true; extraPatterns ? []; extraPaths ? []; }`.
- `extraPatterns` matches on `builtins.baseNameOf path`; defaults filter `-bash-`, `-coreutils-`, `-less-`. `extraPaths` must be strings convertible via `builtins.toPath` (store paths or resolved symlinks).
- The `compactClosure` argument is overridable for advanced use; remove it from the attrset forwarded to `stdenv.mkDerivation`. If you add more private knobs, remember to extend `privateArgNames`.
